       ...
       <div
        ...
        >
           ...
           <SfList>
             <SfListItem v-for="(category, key) in categories" :key="key">
               <SfMenuItem
                  ...
                  :link="localePath(`/c/${category.collection.slug}`)"
                >
                 <template #mobile-nav-icon>
                   &#8203;
                 </template>
         ...
         <SfMegaMenuColumn
          ...
          >
           ...
           <SfScrollable class="results--desktop desktop-only" show-text="" hide-text="">
             <div class="results-listing">
               <SfProductCard
                 ...
                 :key="`${productGetters.getSlug(product)}-${i}-desktop`"
                 :style="{ '--index': i }"
           ...
           <div class="results--mobile smartphone-only">
             <SfProductCard
               v-for="(product, i) in products"
               :key="`${productGetters.getSlug(product)}-${i}-mobile`"
               class="result-card"
               :regular-price="$n(productGetters.getPrice(product).regular, 'currency')"
               :score-rating="productGetters.getAverageRating(product)"
               :image="productGetters.getCoverImage(product)"
         ...
         <div class="action-buttons smartphone-only">
           <SfButton class="action-buttons__button color-light" @click="$emit('close')"> Cancel </SfButton>
         </div>
       </div>
